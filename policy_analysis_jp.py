...
while True:
    x = input('チェックしたいアプリケーションを番号で選んでください： ')
    x = int(x)
    if x in a_dict:
        break
...
while True:
    print('いつからのフローをチェックしたいですか？。開始の年、月、日、時を入力してください')
    print()
    SY = input('Year[YYYY]：')
    SY = str(SY)
    SM = input('Month[MM]：')
...
while True:
    print('いつまでのフローをチェックしたいですか？終了の年、月、日、時を入力してください')
    print()
    EY = input('Year[YYYY]：')
    EY = str(EY)
    EM = input('Month[MM]：')
...
print()
while True:
    c = input('確認したいカテゴリーを番号で入力してください[ 1 ~ 4 ]：')
    c = int(c)
